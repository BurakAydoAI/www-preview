 ...
 *					- /js/added.jsDoc comments and added documentation for the plugin
 *					- code tidy
...
(function (factory) {
  ...
  class TouchSwipe {
    constructor(element, options) {
      ...
      var useTouchEvents = SUPPORTS_TOUCH || SUPPORTS_POINTER || !options.fallbackToMouseEvents, START_EV = useTouchEvents
        ? SUPPORTS_POINTER
          ? SUPPORTS_POINTER_IE10
            ? "MSPointerDown"
            : "pointerdown"
          : "touchstart"
        : "mousedown", MOVE_EV = useTouchEvents
          ? SUPPORTS_POINTER
            ? SUPPORTS_POINTER_IE10
              ? "MSPointerMove"
              : "pointermove"
            : "touchmove"
          : "mousemove", END_EV = useTouchEvents
            ? SUPPORTS_POINTER
              ? SUPPORTS_POINTER_IE10
                ? "MSPointerUp"
                : "pointerup"
              : "touchend"
            : "mouseup", LEAVE_EV = useTouchEvents ? null : "mouseleave", //we manually detect leave on touch devices, so null event here
      ...
      function touchMove(jqEvent) {
        ...
        if (fingerCount === options.fingers ||
          options.fingers === ALL_FINGERS ||
          !touches ||
          hasPinches()) {
